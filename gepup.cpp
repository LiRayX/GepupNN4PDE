//make之后，  mpirun -np 并行的进程数 ./gepup 全局加密次数
...
unsigned int timestep_number = 0; //初始时间步
...
private:
    ...
    void output_image(std::string file_name, unsigned int output_count);
    void print_error();
...
template <int dim>
void GePUP<dim>::output_image(std::string file_name, unsigned int output_count)
{
...
void GePUP<dim>::run_adaptive()
{
    ...
    std::string output_image_name = "Cyflow_Cr03_maxt001_init0_level4_80_10_refineperstep/"; //图片文件名称

    std::ofstream dof_output("dofInfo_Cyflow_Cr03_maxt001_init0_level4_80_10_refineperstep"); //信息文件名称
    ...
    output_image(output_image_name, 0);
